	...
	$.ajax({
		...
		if (res != "") {
    		$('.img-photo-content-user-v').html(`
    			<div id="logo_store" style='background-image: url(../shop/images/store/${res['logo']});background-repeat:no-repeat;background-size: contain;background-position: center;'></div>
    		`);

    		$('.content-upd-logo-store_ttrq').html(`
    			<div style="background-image: url(../shop/images/store/${res['logo']});background-repeat:no-repeat;background-size: contain;background-position: center;"></div>
    		`);
