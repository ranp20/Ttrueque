...
function fetchorder(){
    ...
    try {
        if($token == $systemToken){
            $db = getDB();
            $sqlorder = "SELECT orderid, user_id, fname, mobile, city, address ,status, total, shipping, paymenttype, payid, coupon, couponprice, lat, lng, dtype, dcode, driverid, dname, dmobile, ordertime FROM orders LEFT JOIN deliveryboy on orders.driverid = deliveryboy.id where orders.user_id = :userid ORDER BY orderid DESC";
...
function placeorder() {
    ...
    try {
        ...
        if($systemToken == $token){
            $db = getDB();
            $sqlorder = "INSERT INTO orders(user_id, fname,email, city, address, mobile, paymenttype, payid, total, shipping, coupon, couponprice, dtype, lat, lng)VALUES(:userid, :fname,:email, :city,  :address, :mobile, :paymenttype, :payid, :total, :shipping, :coupon, :couponprice, :dtype, :lat, :lng)";
            $stmtorder = $db->prepare($sqlorder);
